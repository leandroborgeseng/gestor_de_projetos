...
export default function UsersManagement() {
  ...
  // Filtrar usuários baseado em showInactive
  const users = showInactive 
    ? allUsers 
    : allUsers.filter((user: User) => user.active !== false);
  ...
  const handleToggleUserStatus = (user: User) => {
    const isActive = user.active !== false; // Considera true se for undefined ou true
    const action = isActive ? "inativar" : "ativar";
    if (!window.confirm(`Tem certeza que deseja ${action} o usuário "${user.name}"?`)) return;
    toggleUserStatusMutation.mutate({ userId: user.id, active: !isActive });
  };
  ...
  const getRoleColor = (role: string) => {
    const colors: Record<string, string> = {
      SUPERADMIN: "bg-purple-900/40 text-purple-200",
                ...
                {users && users.length > 0 ? (
                  users.map((user: User) => {
                    ...
                    return (
                      <tr 
                        key={user.id} 
                        className={`hover:bg-gray-700 transition-colors ${isInactive ? "opacity-60" : ""}`}
                      >
